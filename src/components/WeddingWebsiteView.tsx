...
export const WeddingWebsiteView: React.FC<WeddingWebsiteViewProps> = ({ weddingId }) => {
  ...
  const getTemplateStyles = () => {
    switch (website.templateId) {
      case 'classic':
        return {
          container: 'font-serif bg-[#fffcf9] text-[#2c3e50]',
          accent: 'text-[#c0392b]',
      ...
      case 'rustic':
        return {
          container: 'font-serif bg-[#fdfaf6] text-[#5d4037]',
          accent: 'text-[#8d6e63]',
      ...
      case 'boho':
        return {
          container: 'font-sans bg-[#faf7f2] text-[#4a4a4a]',
          accent: 'text-[#d4a373]',
      ...
      case 'dark':
        return {
          container: 'font-serif bg-[#1a1a1a] text-[#f5f5f5]',
          accent: 'text-[#d4af37]',
      ...
      case 'vibrant':
        return {
          container: 'font-sans bg-[#fff5f5] text-[#2d3436]',
          accent: 'text-[#ff7675]',
      ...
      case 'editorial':
        return {
          container: 'font-serif bg-white text-black',
      ...
      case 'coastal':
        return {
          container: 'font-sans bg-[#f0f7f9] text-[#2c3e50]',
          accent: 'text-[#3498db]',
    ...
    <div className={`min-h-screen ${styles.container} selection:bg-rose-100 selection:text-rose-900`}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 p-6 flex items-center justify-between ${styles.nav}`}>
        ...
        <div className="hidden md:flex items-center gap-8 text-sm font-bold uppercase tracking-widest opacity-60">
          ...
          <a href="#" className="hover:opacity-100 transition-opacity">Cuándo & Dónde</a>
          {website.sections.find(s => s.type === 'rsvp')?.visible && (
            <button onClick={() => setShowRSVP(true)} className="hover:opacity-100 transition-opacity">RSVP</button>
          )}
        </div>
        {website.sections.find(s => s.type === 'rsvp')?.visible && (
          <button 
      ...
      {/* Hero Section */}
      <section className={`relative min-h-screen flex flex-col items-center justify-center text-center p-8 ${styles.hero}`}>
      ...
      {/* Invitation Section */}
      <section className="py-32 px-8 bg-current/5 text-center">
        <motion.div 
