...
import { UserPlus, Trash2, CheckCircle2, XCircle, Clock, Search, Share2, Copy, ExternalLink, MessageCircle, Download } from 'lucide-react';
...
export const GuestList: React.FC<{ weddingId: string }> = ({ weddingId }) => {
  ...
  const [newColor, setNewColor] = useState('#f43f5e'); // Default rose-500
  ...
  const addGuest = async (e: React.FormEvent) => {
    ...
    const guestData: any = {
      ...
      status: 'invited',
      plusOne: newPlusOne,
      plusOneName: newPlusOne ? newPlusOneName : '',
      color: newColor
    };
    if (newEmail.trim()) {
  ...
  const updatePlusOne = async (id: string, plusOne: boolean) => {
    try {
      await updateDoc(doc(db, `weddings/${weddingId}/guests`, id), { 
        plusOne,
        plusOneName: plusOne ? (guests.find(g => g.id === id)?.plusOneName || '') : ''
      });
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `weddings/${weddingId}/guests/${id}`);
    }
  ...
  const updatePlusOneName = async (id: string, plusOneName: string) => {
    try {
      await updateDoc(doc(db, `weddings/${weddingId}/guests`, id), { plusOneName });
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `weddings/${weddingId}/guests/${id}`);
    }
  };
  ...
  const updateColor = async (id: string, color: string) => {
    try {
      await updateDoc(doc(db, `weddings/${weddingId}/guests`, id), { color });
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `weddings/${weddingId}/guests/${id}`);
    }
  };
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    doc.text('Lista de Invitados - Tu Casamiento', 14, 15);
    ...
    autoTable(doc, {
      head: [['Nombre', 'Email', 'Estado', 'Acompañante', 'Asistentes', 'Dieta']],
      body: tableData,
      startY: 25,
      styles: { fontSize: 8 },
  ...
  const exportToExcel = () => {
    const data = filteredGuests.map(g => ({
      ...
      Email: g.email || '',
      Estado: g.status,
      Acompañante: g.plusOne ? 'Sí' : 'No',
      Asistentes: g.attendeesCount || 0,
      Dieta: g.dietaryRestrictions || '',
      Mensaje: g.message || ''
    }));
    ...
    XLSX.writeFile(workbook, 'lista-invitados.xlsx');
  };
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className="relative group">
            <button 
              className="px-4 py-2 bg-slate-50 text-slate-600 rounded-2xl font-bold text-sm hover:bg-slate-100 transition-all flex items-center gap-2"
            >
      ...
      <form onSubmit={addGuest} className="flex flex-col gap-3 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        ...
        <div className="flex items-center justify-between">
              ...
              {newPlusOne && (
                ...
                >
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-4 py-3 focus:ring-2 focus:ring-rose-500 outline-none transition-all text-sm"
                  />
                </motion.div>
              )}
            </AnimatePresence>
          ...
          <div className="flex flex-col gap-2 ml-4">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">Color en el Mapa</label>
            <div className="flex gap-1.5 flex-wrap max-w-[160px]">
              {presetColors.map(color => (
                <button
                  ...
                  className={`w-6 h-6 rounded-full transition-all border-2 ${newColor === color ? 'border-slate-800 scale-110' : 'border-transparent'}`}
          ...
          <button type="submit" className="bg-rose-500 text-white px-6 py-3 rounded-2xl hover:bg-rose-600 transition-colors shadow-lg shadow-rose-100 flex items-center gap-2 font-bold ml-4 self-end">
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <div className="flex items-center gap-1.5">
                      {presetColors.map(color => (
                        <button
                          key={color}
                          onClick={() => updateColor(guest.id, color)}
                          className={`w-3 h-3 rounded-full transition-all border ${guest.color === color ? 'border-slate-800 scale-125' : 'border-transparent'}`}
                          style={{ backgroundColor: color }}
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      {guest.plusOne && (
                        <input
                          ...
                          onChange={(e) => updatePlusOneName(guest.id, e.target.value)}
                          placeholder="Nombre acompañante..."
                          className="text-[10px] bg-slate-50 border border-slate-100 rounded-lg px-2 py-1 outline-none focus:ring-1 focus:ring-indigo-500 transition-all"
                        />
                      )}
                    </div>
                  </div>
